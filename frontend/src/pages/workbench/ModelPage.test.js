...
describe("ModelPage - save", () => {
    ...
    describe("handleSave", () => {
        ...
        test("retains the created model ID for subsequent saves in the same editor session", async () => {
            ...
            expect(saveModel).toHaveBeenNthCalledWith(1, expect.not.objectContaining({ id: expect.anything() }));
        ...
        test("starting a new model does not reuse the prior saved model ID", async () => {
            ...
            await screen.findByText(/New model. Nothing is saved yet/);
            await saveTheModel();

            expect(saveModel).toHaveBeenCalledTimes(2);
            expect(saveModel).toHaveBeenNthCalledWith(2, expect.not.objectContaining({ id: expect.anything() }));
        });
